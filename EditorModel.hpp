...
// ----------Everything I added below here----------

    // Increases curser_col by 1 to move cursor right
    void move_right();

    // Decreases cursor_col by 1 to move curseor left
    void move_left();
    
    // Increases cursor_line by 1 to move cusor down
    void move_down();

    // Decreases cursor_line by to move cursor up
    void move_up();

    // Inserts a char into the string where the cursor points
    //   The cursor is then moved right
    void insert_char(char c);

    // inserts new line of text at cursor position
    void new_line();

    // Deletes the line the cursor is on
    std::string delete_line();

    // inserts a whole line that was previously deleted by delete_line()
    void insert_line(std::string whole_ass_line, unsigned int line_num, 
                     unsigned int prev_col, unsigned int prev_size);
    
    // Erases a char from the string behind where the cursor points
    //   The cursor is then moved left
    char backspace();
    
    // Moves cursor to beginning of current line
    unsigned int cursor_home();

    // Moves cursor to end of current line
    unsigned int cursor_end();

    // Moves cursor to the column number specified
